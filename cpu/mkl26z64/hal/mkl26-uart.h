//+------------------------------------------------------------------------------------------------+
//| UART peripheral registers for Kinetis MKL26 MCU.                                               |
//|                                                                                                |
//| Author: Joksan Alvarado.                                                                       |
//+------------------------------------------------------------------------------------------------+

#ifndef MKL26_UART_H_
#define MKL26_UART_H_
...
#define UART_BDH_SBR_Msk            0x1F      //Baud rate modulo divisor (MSB)
#define UART_BDH_SBR_Pos            0
#define UART_BDH_SBNS_1             (0 << 5)  //Stop bit number select
#define UART_BDH_SBNS_2             (1 << 5)
#define UART_BDH_RXEDGIE_Disabled   (0 << 6)  //RX input active edge interrupt enable
...
#define UART_BDL_SBR_Msk  0xFF  //Baud rate modulo divisor (LSB)
...
#define UART_C1_DOZEEN_EnWait   (0 << 6)  //Doze enable
#define UART_C1_DOZEEN_DisWait  (1 << 6)
...
#define UART_C2_ILIE_Disabled   (0 << 4)  //Idle line interrupt enable for IDLE
#define UART_C2_ILIE_Enabled    (1 << 4)
#define UART_C2_RIE_Disabled    (0 << 5)  //Receiver interrupt enable for RDRF
#define UART_C2_RIE_Enabled     (1 << 5)
#define UART_C2_TCIE_Disabled   (0 << 6)  //Transmission complete interrupt enable for TC
#define UART_C2_TCIE_Enabled    (1 << 6)
#define UART_C2_TIE_Disabled    (0 << 7)  //Transmit interrupt enable for TDRE
...
#define UART_S1_TC_Msk      0x40      //Transmission complete flag
...
#define UART_S2_BRK13_Short     (0 << 2)  //Break character generation length
...
#define UART_S2_MSBF_LSB        (0 << 5)  //MSB first
#define UART_S2_MSBF_MSB        (1 << 5)
#define UART_S2_RXEDGIF_Msk     0x40      //UART_RX pin active edge interrupt flag
...
#define UART_C3_ORIE_Disabled   (0 << 3)  //Overrun interrupt enable
#define UART_C3_ORIE_Enabled    (1 << 3)
#define UART_C3_TXINV_Normal    (0 << 4)  //Transmit data inversion
#define UART_C3_TXINV_Inverted  (1 << 4)
#define UART_C3_TXDIR_Input     (0 << 5)  //UART_TX pin direction in single wire mode
#define UART_C3_TXDIR_Output    (1 << 5)
#define UART_C3_R9T8_Msk        0x40      //Receive bit 9 / transmit bit 8
#define UART_C3_R9T8_Pos        6
#define UART_C3_R8T9_Msk        0x80      //Receive bit 8 / transmit bit 9
#define UART_C3_R8T9_Pos        7

//UART control register 4 bitfields
#define UART_C4_OSR_Msk         0x1F      //Over sampling ratio
#define UART_C4_OSR_Pos         0
#define UART_C4_M10_8_9Bit      (0 << 5)  //10-bit mode select
#define UART_C4_M10_10Bit       (1 << 5)
...
#define UART_C5_RESYNCDIS_Enabled   (0 << 0)  //Resynchronization disable
#define UART_C5_RESYNCDIS_Disabled  (1 << 0)
#define UART_C5_BOTHEDGE_Rising     (0 << 1)  //Both edge sampling
#define UART_C5_BOTHEDGE_Both       (1 << 1)
#define UART_C5_RDMAE_Disabled      (0 << 5)  //Receiver full DMA enable
#define UART_C5_RDMAE_Enabled       (1 << 5)
#define UART_C5_TDMAE_Disabled      (0 << 7)  //Transmitter DMA enable
#define UART_C5_TDMAE_Enabled       (1 << 7)
